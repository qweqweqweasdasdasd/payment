...
namespace App\Libs\ChenPay;

use App\Libs\ChenPay\PayException\PayException;
...
class AliPay extends Pay
{
    ...
    public function getRefresh()
    {
        ...
        if (!preg_match('/navResult/', $aliPayHtml)) throw new PayException('cookie lose', 445);
        return $this;
    }
    ...
    public function getData($url = false, $syncKey = false)
    {
        ...
        $this->url = $url;
        $this->getRefresh();
        print_r($this->HtmlTwo()->getContents());
        $aliPayHtml = $url ? $this->HtmlOne()->getContents() : $this->HtmlTwo()->getContents();
        if (preg_match('/"failed"/', $aliPayHtml)) {
            $aliPayHtml = !$url ? $this->HtmlOne()->getContents() : $this->HtmlTwo()->getContents();
            if (preg_match('/"failed"/', $aliPayHtml)) throw new PayException('Frequently accessed', 446);
            $this->url = !$url;
        }
        try {
            $this->html = iconv('GBK', 'UTF-8', $aliPayHtml);
        } catch (\Exception $e) {
            throw new PayException('Processing error', 444);
        }
        //dd($this->html);
        return $this;
    }
    ...
    public function DataHandle()
    {
        try {
            ...
            throw new PayException('Parse error', 444);
        }
//        if (isset($this->json['exception_marking'])) throw new PayException('数据出错', 444);
        if (isset($this->json['target'])) throw new PayException('cookie lose', 445);
        //dd($this);
        return $this;
    }
