...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="{{$permission->p_name}}" placeholder="请输入权限" name="p_name">
		</div>
	</div>
    <div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="p_pid" size="1">
				...
				@foreach($Ptree as $v)
				<option value="{{$v['p_id']}}" @if($permission->p_pid == $v['p_id']) selected @endif >{{str_repeat('&nbsp;&nbsp;├─',$v['ps_level']).$v['p_name']}}</option>
				@endforeach
			</select>
			</span> </div> 
	</div>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text"  value="{{$permission->p_c}}" placeholder="请输入控制器"  name="p_c">
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-2">方法：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text"   placeholder="请输入方法"  name="p_a" value="{{$permission->p_a}}">
		</div>
	</div>
    <div class="row cl">
		<label class="form-label col-xs-4 col-sm-2">路由：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text"   placeholder="请输入路由" name="ps_route" value="{{$permission->ps_route}}">
		</div>
	</div>
	<!-- <div class="row cl">
